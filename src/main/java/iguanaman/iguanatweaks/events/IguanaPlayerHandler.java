    ...
    public void onPlayerLogin(PlayerLoggedInEvent event) {
        ...
        if(IguanaConfig.spawnLocationRandomisationMax > 0) {
            ...
            if(!tags.hasKey("IguanaTweaks"))
                tags.setTag("IguanaTweaks", new NBTTagCompound());
    ...
    public static ChunkCoordinates randomiseCoordinates(World world, int x, int z, int rndFactorMin, int rndFactorMax) {
        ...
        for(int attempt = 1; attempt <= 10; ++attempt) {
            ...
            if((rndFactorMax - rndFactorMin) > 0)
                modX += world.rand.nextInt(rndFactorMax - rndFactorMin);
            ...
            if((rndFactorMax - rndFactorMin) > 0)
                modZ += world.rand.nextInt(rndFactorMax - rndFactorMin);
            if(world.rand.nextInt(100) < 50)
                modZ *= -1;
            newZ = z + modZ;
            ...
            if(biomeTypes.length == 1 && biomeTypes[0] == Type.WATER)
                continue;
